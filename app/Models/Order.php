 ...
 * @property float|null $rate_tips
 * @property float|null $shop_tip
 * @property float|null $rate_shop_tip
 * @property float|null $driver_tip
 * @property float|null $rate_driver_tip
 * @property float|null $waiter_tip
 * @property float|null $rate_waiter_tip
 * @property float|null $system_tip
 * @property float|null $rate_system_tip
 ...
 *
 * @property float|null $coupon_sum_price
 * @property float|null $point_histories_sum_price
 * @property float|null $rate_coupon_sum_price
 * @property float|null $rate_point_histories_sum_price
 ...
 *
 * // === Cooking time sahələri ===
 * @property int|null $cooking_minutes
 * @property Carbon|null $cooking_started_at
 * @property Carbon|null $cooking_deadline_at
 *
 * @property Carbon|null $created_at
 * @property Carbon|null $updated_at
 * @property-read int $rate_total_price
 * @property-read float $rate_total_discount
 * @property-read float $order_details_sum_total_price
 * @property-read float $order_details_sum_discount
...
class Order extends Model
{
    ...
    protected $casts = [
        ...
        'address'              => 'array',
        'info'                 => 'array',
        // Cooking time cast-ları
        'cooking_started_at'   => 'datetime',
        'cooking_deadline_at'  => 'datetime',
    ];
    ...
    /** ===== Helpers: Cooking status ===== */
    public function isCooking(): bool
    {
        return $this->status === self::STATUS_COOKING;
    }

    public function isCookingDeadlinePassed(): bool
    {
        return $this->cooking_deadline_at !== null && now()->gte($this->cooking_deadline_at);
    }

    /** ====== Accessors & hesablamalar ====== */
    ...
    public function scopeFilter($query, $filter): void
    {
        ...
        $query
            ...
            ->when(data_get($filter, 'status'),              fn($q,$s)     => $q->where('status', $s))
            ->when(data_get($filter, 'shop_id'),             fn($q,$id)    => $q->where('shop_id', $id))
            ->when(data_get($filter, 'shop_ids'),            fn($q,$ids)   => $q->whereIn('shop_id', (array)$ids))
            ->when(data_get($filter, 'user_id'),             fn($q,$uid)   => $q->where('user_id',   (int)$uid))
            ->when(data_get($filter, 'table_id'),            fn($q,$tid)   => $q->where('table_id',  (int)$tid))
            ->when(data_get($filter, 'table_ids'),           fn($q,$tids)  => $q->where('table_id',  $tids))
            ->when(data_get($filter, 'waiter_id'),           fn($q,$wid)   => $q->where('waiter_id', (int)$wid))
